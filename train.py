"""
train.py
Training engine for N-1 Contingency Recommendation CNN using TensorFlow / Keras.
...
def weighted_bce_loss(pos_weight=15.0):
    """
    Weighted Binary Cross Entropy loss function to handle class imbalance
    in 35x35 contingency recommendation matrices (~2.25% positive labels).
    """
    def loss_fn(y_true, y_pred):
        # y_true: (Batch, 35, 35), y_pred logits: (Batch, 35, 35)
...
class F1ScoreMetric(keras.metrics.Metric):
    """
    Custom Keras metric to track F1-Score from logits across epochs.
    """
    def __init__(self, name='f1_score', threshold=0.0, **kwargs):
...
def train_cnn_model(epochs=25, batch_size=32, lr=1e-3, pos_weight=15.0, dataset_path='data/dataset.npz', checkpoint_dir='checkpoints'):
    """
    Trains the TensorFlow/Keras ContingencyCNN model on 60% train data and evaluates on 20% val data.
    """
    ...
    history_path = os.path.join('data', 'training_history.npz')
    np.savez_compressed(
        history_path,
...
if __name__ == '__main__':
    train_cnn_model(epochs=25, batch_size=32, lr=1e-3, pos_weight=15.0)
