"""
evaluate_gnn.py
Evaluation engine for N-1 Contingency Recommendation GNN on the 1,000 un-seen test set samples.
...
def evaluate_gnn_model(model_path='checkpoints/best_gnn_model.keras', dataset_path='data/dataset.npz', output_json='data/gnn_test_evaluation.json', pos_weight=15.0):
    """
    Evaluates the trained ContingencyGNN model on the 1,000 test set samples.
    """
    if not os.path.exists(model_path):
        raise FileNotFoundError(f"GNN Model checkpoint not found: {model_path}")

    # 1. Load Data
    _, _, (x_test, y_test) = load_dataset(dataset_path)

    # 2. Load Trained GNN Model
    ...
    print(f"\nRunning GNN Model Inference on 1,000 Test Samples...")
    ...
    print("\n" + "=" * 60)
    print("  GNN TEST SET EVALUATION RESULTS (1,000 SAMPLES)")
    print("=" * 60)
    ...
    print("=" * 60)
    print(f"Saved GNN test evaluation metrics to: {output_json}")
